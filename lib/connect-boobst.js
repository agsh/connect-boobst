...
BoobstStore.prototype.get = function (sid, callback) {
	this.getConnection().get(this.global, [sid], function(err, data) {
		if (err) {
			callback();
			return;
		}
...
BoobstStore.prototype.set = function (sid, session, callback) {
	this.getConnection().set(this.global, [sid], JSON.stringify(session), callback);
};
...
BoobstStore.prototype.destroy = function (sid, callback) {
	this.getConnection().kill(this.global, [sid], callback);
};
